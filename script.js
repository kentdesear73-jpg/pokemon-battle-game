// ======================== TEAM BATTLE WITH VICTORY SCREEN ========================
// You choose 3 Pokémon, fight 5 enemies. Switch Pokémon, type advantages.
// After defeating all 5 enemies, shows victory message and restart button.

const allPokemon = {
    groudon: { name: "Groudon", type: "Ground", maxHp: 115, color: "#b87c3a", initial: "G", img: "images/groudon.png",
        attacks: { a1: { name: "Precipice Blades", dmg: [14,22] }, a2: { name: "Lava Plume", dmg: [18,27] } } },
    landorus: { name: "Landorus", type: "Ground", maxHp: 110, color: "#b87c3a", initial: "L", img: "images/landorus.png",
        attacks: { a1: { name: "Sandsear Storm", dmg: [13,21] }, a2: { name: "Earthquake", dmg: [16,25] } } },
    garchomp: { name: "Garchomp", type: "Ground", maxHp: 112, color: "#b87c3a", initial: "C", img: "images/garchomp.png",
        attacks: { a1: { name: "Dragon Claw", dmg: [12,20] }, a2: { name: "Outrage", dmg: [17,28] } } },
    kyogre: { name: "Kyogre", type: "Water", maxHp: 115, color: "#3a7cb8", initial: "K", img: "images/kyogre.png",
        attacks: { a1: { name: "Origin Pulse", dmg: [15,24] }, a2: { name: "Water Spout", dmg: [18,27] } } },
    palkia: { name: "Palkia", type: "Water", maxHp: 110, color: "#3a7cb8", initial: "P", img: "images/palkia.png",
        attacks: { a1: { name: "Spacial Rend", dmg: [14,22] }, a2: { name: "Hydro Pump", dmg: [17,28] } } },
    greninja: { name: "Greninja", type: "Water", maxHp: 105, color: "#3a7cb8", initial: "G", img: "images/greninja.png",
        attacks: { a1: { name: "Water Shuriken", dmg: [12,19] }, a2: { name: "Hydro Cannon", dmg: [18,30] } } },
    charizard: { name: "Charizard", type: "Fire", maxHp: 110, color: "#e05a2a", initial: "C", img: "images/charizard.png",
        attacks: { a1: { name: "Flamethrower", dmg: [13,21] }, a2: { name: "Blast Burn", dmg: [18,30] } } },
    moltres: { name: "Moltres", type: "Fire", maxHp: 112, color: "#e05a2a", initial: "M", img: "images/moltres.png",
        attacks: { a1: { name: "Sky Attack", dmg: [14,22] }, a2: { name: "Overheat", dmg: [19,31] } } },
    blaziken: { name: "Blaziken", type: "Fire", maxHp: 110, color: "#e05a2a", initial: "B", img: "images/blaziken.png",
        attacks: { a1: { name: "Blaze Kick", dmg: [13,21] }, a2: { name: "Flare Blitz", dmg: [17,28] } } }
};

const opponentsList = [
    { name: "Mewtwo", type: "Psychic", maxHp: 115, initial: "M", color: "#9b59b6", img: "images/mewtwo.png",
        attacks: { a1: [12,20], a2: [15,25] } },
    { name: "Rayquaza", type: "Dragon", maxHp: 120, initial: "R", color: "#2ecc71", img: "images/rayquaza.png",
        attacks: { a1: [14,22], a2: [17,27] } },
    { name: "Lugia", type: "Flying", maxHp: 118, initial: "L", color: "#3498db", img: "images/lugia.png",
        attacks: { a1: [11,19], a2: [14,23] } },
    { name: "Zacian", type: "Fairy", maxHp: 112, initial: "Z", color: "#e67e22", img: "images/zacian.png",
        attacks: { a1: [13,21], a2: [16,26] } },
    { name: "Darkrai", type: "Dark", maxHp: 108, initial: "D", color: "#34495e", img: "images/darkrai.png",
        attacks: { a1: [12,20], a2: [15,24] } }
];
...
function playerAction(actionType) {
    ...
    if (actionType === "heal") {
        const healAmount = Math.min(HEAL_AMOUNT, active.maxHp - active.hp);
        if (healAmount > 0) {
            active.hp += healAmount;
            addLog(`${active.name} heals ${healAmount} HP. HP now ${active.hp}/${active.maxHp}`);
            updateUI();
        } else {
            addLog(`HP is already full. Heal had no effect.`);
        }
    } else {
        const attackKey = actionType === "attack1" ? "a1" : "a2";
    ...
    if (currentEnemy.hp <= 0) {
        ...
        const defeatedCount = 5 - enemiesRemaining.length;
        enemiesDefeatedSpan.textContent = `${defeatedCount} / 5`;
...
function startBattle() {
    ...
    currentEnemy = { ...enemiesRemaining[0] };
    currentEnemy.hp = currentEnemy.maxHp;
    enemiesDefeatedSpan.textContent = `0 / 5`;
...
attack1Btn.onclick = () => playerAction("attack1");
attack2Btn.onclick = () => playerAction("attack2");
